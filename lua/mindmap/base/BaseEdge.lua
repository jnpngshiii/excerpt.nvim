...
function BaseEdge:upgrade()
  ...
  while current_version < latest_version do
    ...
    if upgrade_func then
      ...
      if not ok then
        -- stylua: ignore
        logger.error("Upgrade from `v" .. current_version .. "` to `v" .. next_version .. "` failed: " .. result)
      end
      -- stylua: ignore
      logger.info("Upgrade from `v" .. current_version .. "` to `v" .. next_version .. "` succeeded.")
    else
      -- stylua: ignore
      logger.warn("Upgrade from `v" .. current_version .. "` to `v" .. next_version .. "` failed: " .. "Cannot find upgrade function, forced upgrade.")
    end
...
function BaseEdge:check_health()
  ...
  if not #issues == 0 then
    logger.error("Health check failed.", issues)
  end
end
